...
string str;
stack<int> stek;
int Velicina, Jedinice, Dvojke, Trojke, Cetvorke, Brojac, brHeuristike, slobodni, priv1, priv2, kvadrat, br_ispisa;
bool temp;
struct slog
{
   int koordx;
...
void HEUR_REKURZIJA(int a, int b) /// DESPAGETIRAN
{
   /// PROBAJ SA BRODOM, DODAJ ODSECANJE DA NE MOZE AKO USLOVI NISU ISPUNJENI, TJ. AKO JE OVO > OD USLOVA
   KopijaMatrice[a][b] = 'p';
   ...
   if(VALIDAN_ZBIR(a, b))
   {
      ...
      for(int i=1; i<=brHeuristike; i++)
      {
         cout << Heuristika[i].koordx << " " << Heuristika[i].koordy << " " << i << "\n";
         HEUR_REKURZIJA(Heuristika[i].koordx, Heuristika[i].koordy);
         KopijaMatrice[Heuristika[i].koordx][Heuristika[i].koordy] = '.';
      }
   }
